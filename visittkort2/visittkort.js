...
const personListe = document.getElementById("navnVelger");

personsDatabaseDB.once("value")
  .then((snapshot) => {
    const data = snapshot.val();

    // Iterate through each node in the database
    for (const nodeName in data) {
        console.log(nodeName);
      if (data.hasOwnProperty(nodeName)) {
        const navnNode = document.createElement("option");
        navnNode.innerHTML = nodeName;
        navnNode.value = nodeName;
        navnNode.id = nodeName;
